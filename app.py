...
# Only show consultation tabs if no messages or only system/user initial question
    ...
    with tab1:
        ...
        if st.button("Cast the Hexagram", type="primary", use_container_width=True):
            if not question_iching.strip():
                ...
                st.session_state.messages.append({"role": "user", "content": question_iching})
    ...
    if prompt := st.chat_input("Ask a follow-up question..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("The Master is contemplating..."):
                response = st.session_state.interpreter.chat(st.session_state.messages)
